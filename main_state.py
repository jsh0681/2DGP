...
class Object:
    ...
    def insert(self, char, time):
        ...
        if char == 'a':
            if self.image == None:
                self.image = load_image("Graphics\\pipe.png")
                self.a = 30
                self.b = 30
                self.state = 0
        elif char == 'e':
            if self.image == None:
                self.image = load_image("Graphics\\greenstar.png")
                self.a = 15
                self.b = 15
                self.y = 155
                self.state = 1
            ...
            if self.image == None:
                self.image = load_image("Graphics\\pipe2.png")
                self.a =20
                self.b =40
                self.state = 0
            ...
            if self.image == None:
                self.image = load_image("Graphics\\rock.png")
                self.a =40
                self.b =40
                self.state = 0
            ...
            if self.image == None:
                self.image = load_image("Graphics\\uppipe.png")
                self.y = 380
                self.a = 50
                self.b = 225
                self.state = 0
...
background_width = 2396 # in tiles
background_height = 599  # in tiles
...
class Grass:
    ...
    def __init__(self,w,h):
        ...
        self.left =0
        self.speed = 0
        self.image = load_image('Graphics\\bottom.png')
        self.screen_width = w
        self.screen_height = h
    def update(self,frame_time):
        self.speed = Grass.RUN_SPEED_PPS
...
def enter():
    global hero, grass, background, mainBGM, file, object_list, index, line, UI, stars
    file = open("data.txt", 'r')  # 파일을 읽기 모드로 엶
...
def exit():
    global hero, grass, background, mainBGM,stars
    del(hero,grass,background, mainBGM,stars)
    pass
...
def handle_events(frame_time):
    global file, start_time, current_time, UI,hero , mainBGM
    events = get_events()
    for event in events:
        if event.type == SDL_QUIT:
            ...
            mainBGM.pausesong()
            game_framework.change_state(maker_state)
        elif UI.score > 2000:
            mainBGM.pausesong()
...
def update(frame_time):
    global object_list, UI, hero, stars, j, time1, t1
    ...
    for object in object_list:
        if collide(object, hero):
            if object.state == 0:
                ...
                UI.score +=30
    for star in stars:
        if collide(hero, star):
            star.playsong()
            UI.score += 25
            stars.remove(star)
    for star in stars:
        for object in object_list:
            if collide(object,star):
                stars.remove(star)
...
def draw(frame_time):
    global hero,grass,star,background, object_list,UI,star
    handle_events(frame_time)
    clear_canvas()
    background.draw(frame_time)
    grass.draw(frame_time)
    for star in stars:
        star.draw(frame_time)
    for i in range(len(object_list)):
