...
def enter():
    global hero, grass, background, mainBGM, file, object_list, index, line, UI
...
def handle_events(frame_time):
    global file, start_time, current_time,UI
    events = get_events()
    for event in events:
    ...
    for object in object_list:
        if collide(object, hero):
            if object.state == 0:
                ...
                UI.score +=500
def draw(frame_time):
    global hero,grass,star,background, object_list,UI
