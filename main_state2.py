...
class Object:
    ...
    def insert(self, char, time):
        ...
        if char == 'a':
            if self.image == None:
                self.image = load_image("Graphics\\image248.png")
                self.a = 25
                self.b = 20
                self.state = 0
        elif char == 'e':
            if self.image == None:
                self.image = load_image("Graphics\\redstar.png")
                self.a = 15
                self.b = 15
                self.y = 155
                self.state = 1
            ...
            if self.image == None:
                self.image = load_image("Graphics\\image402.png")
                self.a =20
                self.b =40
                self.state = 0
        elif char == 'f':
            ...
            if self.image == None:
                self.image = load_image("Graphics\\image541.png")
                self.a = 10
                self.b = 40
                self.state = 0
        elif char == 'g':
            ...
            if self.image == None:
                self.image = load_image("Graphics\\image337.png")
                self.y = 380
                self.a = 50
                self.b = 225
                self.state = 0
...
class BackGround:
    ...
    def __init__(self,w,h):
        ...
        self.left =0
        self.speed = 0
        self.image = load_image('Graphics\\factory.png')
        self.screen_width = w
...
class Grass:
    ...
    def __init__(self,w,h):
        ...
        self.left =0
        self.speed = 0
        self.image = load_image('Graphics\\Stonewall.png')
        self.screen_width = w
        self.screen_height = h
    def update(self,frame_time):
        self.speed = Grass.RUN_SPEED_PPS
...
def enter():
    global hero, grass, background, mainBGM, file, object_list, index, line, UI,redstars
    file = open("data2.txt", 'r')  # 파일을 읽기 모드로 엶
...
def exit():
    global hero, grass, background, mainBGM,redstars
    del(hero,grass,background, mainBGM,redstars)
    pass
...
def handle_events(frame_time):
    ...
    for event in events:
        if event.type == SDL_QUIT:
            ...
            mainBGM.pausesong()
            game_framework.change_state(maker_state2)
        elif UI.score<800:
            mainBGM.pausesong()
...
def update(frame_time):
    global object_list, UI, hero,redstars,j,time2,t2
    ...
    if j >= 5:
        redstars.append(Redstar())
        j = 0

    for redstar in redstars:
        redstar.update(redstars)
    ...
    for redstar in redstars:
        if collide(hero, redstar):
            redstar.playsong()
            UI.score += 20
            redstars.remove(redstar)

    for redstar in redstars:
        for object in object_list:
            if collide(object,redstar):
                redstars.remove(redstar)
    if time2>=100:
        t2+=0.2
        time2=0
...
def draw(frame_time):
    global hero,grass,star,background, object_list,UI,redstar
    clear_canvas()
